...
class GithubProfile(models.Model):
    ...
    user = models.OneToOneField('authentication.User', on_delete=models.CASCADE, related_name='github_profile')
    github_username = models.CharField(max_length=255, unique=True)
    github_id = models.BigIntegerField(unique=True)
    access_token = models.TextField(blank=True, null=True)  # Use TextField for longer tokens
    ...
    sync_frequency = models.CharField(max_length=20, choices=SYNC_FREQUENCY_CHOICES, default='daily')
    
    # Repository Filtering
    show_forked_repos = models.BooleanField(default=False)
    excluded_repos = models.JSONField(default=list, blank=True)  # List of repo names to exclude
...
class Repository(models.Model):
    ...
    full_name = models.CharField(max_length=255)  # owner/repo-name
    ...
    languages = models.JSONField(default=dict, blank=True)  # Language breakdown
    topics = models.JSONField(default=list, blank=True)  # Repository topics/tags
    ...
    custom_description = models.TextField(blank=True, null=True)  # Override GitHub description
...
class Commit(models.Model):
    ...
    github_profile = models.ForeignKey(GithubProfile, on_delete=models.CASCADE, related_name='commits')
...
class GitHubSyncLog(models.Model):
    ...
    github_profile = models.ForeignKey(GithubProfile, on_delete=models.CASCADE, related_name='sync_logs')
    ...
    def __str__(self):
        return f"{self.github_profile.github_username} - {self.sync_type} - {self.status}"
